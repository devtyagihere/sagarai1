...
export default function Home() {
  ...
  const handleAnalyse = async () => {
    ...
    if (
      ...
      setAnalysisError(
        "Please complete all shipment details before analysing."
      );
      setAnalysing(false);
      return;
    }
    ...
    try {
      /*
       * Save the shipment for the currently logged-in user.
       */
      const savedShipment = saveShipment(formData);

      /*
       * Create a dynamic notification using the
       * actual shipment entered by the user.
       */
      addNotification({
        type: "shipment",
        title: "Shipment analysed",
        message: `Your ${formData.origin} → ${formData.destination} shipment has been added to your workspace.`,
      ...
      /*
       * Send the shipment to the backend analysis engine.
       */
      const response = await fetch(
        "http://127.0.0.1:8001/api/scenario",
        {
          method: "POST",
          headers: {
      ...
      /*
       * Move the user to the overview after analysis.
       */
      navigate("/overview");
    } catch (error) {
      ...
      if (
        ...
        setAnalysisError(
          "Shipment saved successfully, but the freight analysis backend is currently unavailable."
        );
      } else {
        ...
        <div className="form-action">
          ...
          {analysisResult && (
            <div className="analysis-success">
              ✓ Shipment received by the analysis engine
            </div>
          )}
